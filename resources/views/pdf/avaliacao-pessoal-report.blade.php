...
@section('content')
    ...
    <div class="cards">
        <div class="card"><div class="card-label">Media geral consolidada</div><div class="card-value">{{ $formatScore((float) $mediaDeTodos) }}</div></div>
        <div class="card"><div class="card-label">Soma das medias individuais limitada a 3</div><div class="card-value">{{ number_format((float) $somaMediasIndividuais, 2, ',', '.') }}</div></div>
        <div class="card"><div class="card-label">Usuarios avaliadores</div><div class="card-value">{{ $totalAvaliadores }}</div></div>
        <div class="card"><div class="card-label">Avaliacoes registradas</div><div class="card-value">{{ $totalAvaliacoes }}</div></div>
    </div>
    ...
    @foreach (['semanal', 'mensal', 'semestral'] as $comparisonKey)
        ...
        <div class="comparison-box">
            ...
            <table>
                <thead><tr><th>Metrica</th><th>Periodo atual</th><th>Periodo anterior</th><th>Variacao</th></tr></thead>
                <tbody>
                    <tr><td>Media das avaliacoes</td><td>{{ $formatScore((float) $comparison['raw_current']) }}</td><td>{{ $formatScore((float) $comparison['raw_previous']) }}</td><td>{{ ($comparison['raw_delta'] >= 0 ? '+' : '') . number_format((float) $comparison['raw_delta'], 2, ',', '.') }}</td></tr>
                    <tr><td>Media consolidada dos avaliadores</td><td>{{ $formatScore((float) $comparison['consolidated_current']) }}</td><td>{{ $formatScore((float) $comparison['consolidated_previous']) }}</td><td>{{ ($comparison['consolidated_delta'] >= 0 ? '+' : '') . number_format((float) $comparison['consolidated_delta'], 2, ',', '.') }}</td></tr>
                </tbody>
            </table>
        </div>
    @endforeach
    ...
    @forelse ($usuarios as $item)
        <div class="evaluator-card">
            <div class="evaluator-head">
                <div class="evaluator-photo-wrap"><div class="evaluator-photo-empty">{{ str($item['user']?->name ?? '?')->substr(0, 1)->upper() }}</div></div>
            ...
            <div class="evaluator-summary">
                <div class="evaluator-summary-item"><div class="card-label">Qtd. votos</div><div class="card-value">{{ $item['quantidade'] }}</div></div>
                <div class="evaluator-summary-item"><div class="card-label">Media individual</div><div class="card-value">{{ $formatScore((float) $item['media']) }}</div></div>
                <div class="evaluator-summary-item"><div class="card-label">Ultimo voto</div><div class="card-value" style="font-size: 11px;">{{ $item['ultima_avaliacao']?->created_at?->format('d/m/Y') ?? '-' }}</div></div>
            </div>
            <div class="evaluator-criteria">@foreach ($item['criterios'] as $label => $media)<div class="evaluator-criteria-item"><div class="evaluator-criteria-label">{{ $label }}</div><div class="evaluator-criteria-value">{{ $formatScore((float) $media) }}</div></div>@endforeach</div>
        </div>
    @empty
    ...
    <table>
        ...
        <tbody>
            ...
            @empty
                <tr><td colspan="8">Nenhuma avaliacao registrada para este acolhido.</td></tr>
            @endforelse
        </tbody>
    </table>
    ...
    <table>
        ...
        <tbody>
            <tr><td>Media consolidada</td><td>Somatorio das medias dos avaliadores dividido pela quantidade de avaliadores.</td></tr>
        </tbody>
    </table>
@endsection
