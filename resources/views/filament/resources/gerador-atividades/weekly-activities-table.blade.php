...
<div class="space-y-4">
    <div class="grid gap-4 md:grid-cols-3">
        ...
        <div class="rounded-2xl border border-emerald-100 bg-gradient-to-br from-emerald-50 to-white p-4 shadow-sm">
            ...
            <div class="mt-2 text-base font-semibold text-gray-900">{{ count($acolhidos) }}</div>
            <div class="mt-2 text-xs text-gray-600">{{ $acolhidos === [] ? 'Nenhum acolhido vinculado.' : implode(', ', array_slice($acolhidos, 0, 4)) . (count($acolhidos) > 4 ? ' +' . (count($acolhidos) - 4) : '') }}</div>
        </div>
        <div class="rounded-2xl border border-amber-100 bg-gradient-to-br from-amber-50 to-white p-4 shadow-sm">
            ...
            <div class="mt-2 text-base font-semibold text-gray-900">{{ count($rows) }}</div>
