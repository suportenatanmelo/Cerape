...
class DashboardStatsOverviewWidget extends StatsOverviewWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $consultasHoje = Agenda::whereBetween('data', [$start, $end])->count();
        $agendamentosHoje = Agenda::whereBetween('data', [$start, $end])->count();
        $altasMes = Acolhido::where('ativo', false)
            ->whereBetween('updated_at', [$today->copy()->startOfMonth(), $today->copy()->endOfMonth()])
            ->count();
        $estoqueBaixo = 0;
        $funcionariosAtivos = User::where('active_status', true)->count();
        ...
        return [
            ...
            Stat::make('Novos no mês', (string) $novosMes)->icon('heroicon-o-plus-circle')->color('info')->description('Criados no período'),
            Stat::make('Consultas de hoje', (string) $consultasHoje)->icon('heroicon-o-calendar-days')->color('warning')->description('Agenda do dia'),
            Stat::make('Agendamentos hoje', (string) $agendamentosHoje)->icon('heroicon-o-clock')->color('gray')->description('Compromissos do dia'),
            ...
            Stat::make('Funcionários ativos', (string) $funcionariosAtivos)->icon('heroicon-o-briefcase')->color('secondary')->description('Equipe habilitada'),
