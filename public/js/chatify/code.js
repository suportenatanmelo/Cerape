 ...
 *-------------------------------------------------------------
 */
const pusherEnabled = !!chatify.pusher.key && typeof Pusher !== "undefined";
let pusher = null;

if (pusherEnabled) {
  Pusher.logToConsole = chatify.pusher.debug;
  pusher = new Pusher(chatify.pusher.key, {
...
var channel = pusherEnabled ? pusher.subscribe(`${channelName}.${auth_id}`) : null;
var clientSendChannel;
var clientListenChannel;

function initClientChannel() {
  if (pusherEnabled && getMessengerId()) {
...
if (channel) channel.bind("messaging", function (data) {
...
if (clientListenChannel) clientListenChannel.bind("client-typing", function (data) {
  if (data.from_id == getMessengerId() && data.to_id == auth_id) {
    data.typing == true
...
if (clientListenChannel) clientListenChannel.bind("client-seen", function (data) {
  if (data.from_id == getMessengerId() && data.to_id == auth_id) {
    if (data.seen == true) {
      $(".message-time")
        .find(".fa-check")
...
if (clientListenChannel) clientListenChannel.bind("client-contactItem", function (data) {
  if (data.to == auth_id) {
    if (data.update) {
...
if (clientListenChannel) clientListenChannel.bind("client-messageDelete", function (data) {
  $("body").find(`.message-card[data-id=${data.id}]`).remove();
});
// listen on delete conversation event
if (clientListenChannel) clientListenChannel.bind("client-deleteConversation", function (data) {
...
var activeStatusChannel = pusherEnabled ? pusher.subscribe("presence-activeStatus") : null;

// Joined
if (activeStatusChannel) activeStatusChannel.bind("pusher:member_added", function (member) {
  setActiveStatus(1);
...
// Leaved
if (activeStatusChannel) activeStatusChannel.bind("pusher:member_removed", function (member) {
  setActiveStatus(0);
  $(".messenger-list-item[data-contact=" + member.id + "]")
    .find(".activeStatus")
    .remove();
...
$(document).ready(function () {
  ...
  if (pusherEnabled && pusher && channel) {
